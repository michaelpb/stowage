...
def parse_args(argv):
    ...
    parser.add_argument('-v', '--verbose', help='increase output verbosity',
                        action='store_true')
    parser.add_argument('-s', '--source', help='stowage source directory',
                        default=os.environ.get('STOWAGE_SOURCE', '~/dotfiles'))
    parser.add_argument('-d', '--destination', help='stowage destination directory',
                        default=os.environ.get('STOWAGE_DESTINATION', '~'))
    parser.add_argument('-b', '--backup', help='stowage backup directory',
                        default=os.environ.get('STOWAGE_BACKUP', '~/.config/stowage/backup/'))
...
def directory_walk(source_d, destination_d):
    ...
    for dirpath, dirnames, filenames in os.walk(source_d):
        ...
        for filename in filenames:
            ...
            full_destination_path = os.path.join(destination_d, munge_path(suffix))
...
def main(args):
    ...
    for source_d in source_directories(args):
        for source, destination in needed_symlink_walk(source_d, args.destination):
            # Backup logic
            if not args.skip_backup and os.path.exists(destination):
                backup_path = get_backup_path(args, destination)
                if _is_verbose:
                    print('Backing up {0} -> {1}'.format(destination, backup_path))
